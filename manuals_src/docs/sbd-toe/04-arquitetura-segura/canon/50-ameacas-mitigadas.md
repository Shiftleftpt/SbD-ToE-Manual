...
# 🔐 Ameaças Mitigadas — Capítulo 04: Arquitetura Segura {arquitetura-segura:canon:ameacas-mitigadas}
...
## 🧱 Categoria 1 – Falhas de segmentação e isolamento arquitetural {arquitetura-segura:canon:ameacas-mitigadas#categoria_1__falhas_de_segmentacao_e_isolamento_arquitetural}
...
| Mistura de dados e controlo na mesma zona     | OWASP SAMM Design / ISO 27034    | Lógica e dados críticos partilham contexto de execução        | Separação arquitetural explícita entre responsabilidades                           | `addon/04-diagramas-referencia.md`          | ✅                                     |
...
## 🔎 Categoria 2 – Deficiências na modelação arquitetural {arquitetura-segura:canon:ameacas-mitigadas#categoria_2__deficiencias_na_modelacao_arquitetural}
...
| Arquitetura não revê mecanismos de fallback   | OSC&R Design                    | Falta de arquitetura para falhas, limites, fail-safe     | Requisitos arquiteturais para failover, contingência e segurança por defeito      | `addon/01-catalogo-requisitos.md`             | ❌ Cap. 10                             |

---

## 🧪 Categoria 3 – Validação e evolução arquitetural negligenciada {arquitetura-segura:canon:ameacas-mitigadas#categoria_3__validacao_e_evolucao_arquitetural_negligenciada}

| Ameaça                                         | Fonte                              | Como surge                                             | Como a prática mitiga                                                          | Controlos associados                      | 🧩 Mitigada apenas por este capítulo? |
|------------------------------------------------|-------------------------------------|--------------------------------------------------------|---------------------------------------------------------------------------------|-------------------------------------------|----------------------------------------|
| Arquitetura nunca revista                     | ISO 27034 / SSDF PW.5              | Uma vez desenhada, nunca mais é validada              | Integração com ciclo de vida e validações periódicas por risco                 | `canon/15-aplicacao-lifecycle.md`        | ❌ Cap. 01                             |
| Alterações estruturais sem revalidação        | CAPEC-137 / OWASP SAMM             | Mudança crítica de fluxo não revê modelo arquitetural | Validação arquitetural forçada por evento ou por sprint                        | `addon/05-validacao.md`                  | ✅                                     |
...
| Exceções arquiteturais sem rasto              | SSDF RM.1 / ISO 27005 / DSOMM – Documentation           | Casos “especiais” não são revistos ou documentados     | Gestão formal de exceções com validação técnica e aprovação                      | `addon/03-excecoes.md`                   | ❌ Cap. 14                             |

---

## 🔄 Categoria 4 – Ausência de rastreabilidade e requisitos arquiteturais {arquitetura-segura:canon:ameacas-mitigadas#categoria_4__ausencia_de_rastreabilidade_e_requisitos_arquiteturais}
...
| Impossibilidade de mapear decisões a controlos| OWASP SAMM Design / BSIMM AA1.6    | Decisões arquiteturais não se traduzem em controlos   | Rastreabilidade entre decisão → requisito → controlo                          | `addon/06-rastreabilidade.md`                | ✅                                     |
...
## 🧠 Categoria 5 – Incoerência entre arquitetura e risco {arquitetura-segura:canon:ameacas-mitigadas#categoria_5__incoerencia_entre_arquitetura_e_risco}

| Ameaça                                      | Fonte                             | Como surge                                           | Como a prática mitiga                                                              | Controlos associados                       | 🧩 Mitigada apenas por este capítulo? |
|---------------------------------------------|------------------------------------|------------------------------------------------------|-------------------------------------------------------------------------------------|--------------------------------------------|----------------------------------------|
| Aplicações L1 tratadas como críticas        | OSC&R / SSDF PW.1                 | Design arquitetural sem relação com classificação     | Validação arquitetural proporcional ao risco (ex: app L1 pode partilhar recursos)  | `canon/15-aplicacao-lifecycle.md`         | ❌ Cap. 01                             |
| Sobredimensionamento de segurança arquitetural | ISO 27005 / OWASP SAMM          | Investimento excessivo em zonas de baixo risco       | Aplicação da matriz de risco e critérios arquiteturais por perfil                  | `addon/02-casos-praticos.md`              | ✅                                     |
...
## ✅ Conclusão {arquitetura-segura:canon:ameacas-mitigadas#conclusao}
