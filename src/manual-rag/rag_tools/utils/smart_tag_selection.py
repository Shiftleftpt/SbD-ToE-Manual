...
def categorize_tags(tags: List[str]) -> Dict[str, List[str]]:
    ...
    categories = {
        'mandatory': [],      # tipo:*, tema:* (cat_* and grp_* are now in frontmatter fields)
    ...
    for tag in tags:
        if ':' in tag:  # tipo:*, tema:* - keep these as mandatory
...
1. Always keep mandatory tags (tipo:*, tema:* only)
2. Category and group info is in 'categoria' and 'group' fields (not in tags)
3. Keep existing tags (they have user intent)
4. Add high-confidence suggestions (>70%) to fill remaining slots
5. Target: 5-7 tags total per document (readable, visually clean)
...
This ensures:
✓ Docusaurus renders categoria/group pills from dedicated fields
✓ Tags remain focused and readable (7 tags max)
